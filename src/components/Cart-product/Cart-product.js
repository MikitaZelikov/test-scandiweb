...
class CartProduct extends Component {
  ...
  getPrice() {
    ...
    return prices.find((item) => (item.currency === activeCurrency)).amount;
  }
  ...
  render() {
    ...
    return (
      <li className={isCart ? 'product-item' : 'product-item product-item--dropdown'}>
        {this.renderDescription(brand, name, attributes, activeCurrency, isCart)}
        {this.renderPreview(isCart, amount, gallery, counterImg, isOnlyOneImg, additionalId)}
      </li>
    );
  }

  renderDescription(brand, name, attributes, activeCurrency, isCart) {
    return (
      <div className="product-description">
        ...
        <p className={isCart ? 'product-description__amount'
          ...
          {`${getSymbolFromCurrency(activeCurrency)}${this.getPrice()}`}
        </p>
  ...
  renderPreview(isCart, amount, gallery, counterImg, isOnlyOneImg, additionalId) {
    return (
      <div className="product-preview" id="product-preview">
        <div className="product-preview__counter">
          ...
          <span className={isCart ? 'product-preview__counter--amount' : 'product-preview__counter--amount--dropdown'}>
            {amount}
          </span>
          <button
        ...
        <div className="product-delete-icon" onClick={() => this.handleDeleteClick(additionalId)}>
          <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M26.9844 11.9844V14H13.0156V11.9844H16.4844L17.5156 11H22.4844L23.5156 11.9844H26.9844ZM14
                26.9844V14.9844H26V26.9844C26 27.5156 25.7969 27.9844 25.3906 28.3906C24.9844 28.7969 24.5156 29
                23.9844 29H16.0156C15.4844 29 15.0156 28.7969 14.6094 28.3906C14.2031 27.9844 14 27.5156 14 26.9844Z"
              fill="#666666" />
